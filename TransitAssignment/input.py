...
def input_network():
    ...
    with open (file = 'network.csv') as f:
        """
            output to check the network 
        """
        pass
        print("Node,OutLink,LinkTime,LinkTail,LinkHead,LinkLine,Fre")
        for node in graph.nodes:
            for link in node.out_links:
                print("{0},{1},{2},{3},{4},{5},{6}".format(node.name,link.name,link.tail,link.head,link.lines[0].name,link.lines[0].fre))
            pass
